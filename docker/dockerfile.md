...
Dockerfile 中每一个指令都会建立一层（不超过 127 层），RUN 也不例外，所以应尽量简化层数，每一层只添加真正需要添加的东西等，任何无关的东西都应该清理掉。
...
- 类似命令行：`COPY [--chown=<user>:<group>] <源路径>... <目标路径>`
- 类似函数调用：`COPY [--chown=<user>:<group>] ["<源路径1>",... "<目标路径>"]`
